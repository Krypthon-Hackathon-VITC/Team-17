...
function App() {
  ...
  const loadBlockchainData = async () => {
    ...
    const dappazon = new ethers.Contract("0x5FbDB2315678afecb367f032d93F642f64180aa3", Dappazon.abi, provider)
    ...
    const fruitVeg = items.filter((item) => item.category === 'fruits and vegetables')
  ...
  return (
    <div>
      ...
      {rabi && kharif && fruitVeg && (
        <>
          <Section title={"Kharif Crops"} items={kharif} togglePop={togglePop} />
          <Section title={"Rabi Crops"} items={rabi} togglePop={togglePop} />
          <Section title={"Fruits and Vegetable Set"} items={fruitVeg} togglePop={togglePop} />
        </>
      )}
